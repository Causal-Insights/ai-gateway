...
SEEDANCE_2_5_API_KEY_SECRET="${SEEDANCE_2_5_API_KEY_SECRET:-SEEDANCE_2_5_API_KEY}"
...
if gcloud secrets describe "${SEEDANCE_2_5_API_KEY_SECRET}" --project="${PROJECT_ID}" >/dev/null 2>&1; then
  SET_SECRETS+=",SEEDANCE_2_5_API_KEY=${SEEDANCE_2_5_API_KEY_SECRET}:latest"
else
  echo "==> Skipping ${SEEDANCE_2_5_API_KEY_SECRET}: secret is not in Secret Manager (Seedance 2.5 stays unbound)."
fi
...
DEPLOY_ARGS=(
  ...
  --update-env-vars "SEEDANCE_SYNC_WAIT_S=${SEEDANCE_SYNC_WAIT_S},SEEDANCE_POLL_TIMEOUT_S=${SEEDANCE_POLL_TIMEOUT_S},RUN_LITELLM_MIGRATIONS=${RUN_LITELLM_MIGRATIONS},GROK_VIDEO_15_VIDEO_OPERATIONS_VERIFIED=${GROK_VIDEO_15_VIDEO_OPERATIONS_VERIFIED}"
